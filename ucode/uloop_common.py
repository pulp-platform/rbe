...
def uloop_state_machine(loops, curr_state, verbose=False):
...
def uloop_execute(state, code, registers):
    ...
    if code[addr]['op_sel']:
        print(addr, loop, op, idx)
        print(code[addr]['a'], code[addr]['b'])
        print(len(new_registers), len(registers))
